...
#if defined(NDEBUG)
# error "Yusicoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8766daef9281e92549e996be500f079914313d783980bb2b6b4e4e6cba84148b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Yusicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Yusicoin Signed Message:\n";
...
{
    // Yusicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Yusicoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; //3.5 * 24 * 60 * 60; // Yusicoin: 3.5 days
static const int64 nTargetSpacing = 1 * 30; //2.5 * 60; // Yusicoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Yusicoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Yusicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8766daef9281e92549e996be500f079914313d783980bb2b6b4e4e6cba84148b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "build by yusi";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xd067816c29ef37230816d53fc28e4797360c205a30a731e92edb69a22861cbe7"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Yusicoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("YusicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("YusicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static YusicoinMiner(CWallet *pwallet)
{
    printf("YusicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("yusicoin-miner");
    ...
    try { loop {
        ...
        printf("Running YusicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("YusicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&YusicoinMiner, pwallet));
}
